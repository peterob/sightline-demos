...
const GradientShell = ({ children, className = "" }) => (
  <div
    className={cn(
      "relative overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950 shadow-2xl shadow-indigo-950/20",
      className
    )}
  >
...
const SoftCard = ({ children, className = "" }) => (
  <div className={cn("rounded-[1.65rem] border border-slate-200 bg-white shadow-sm shadow-slate-200/60", className)}>
    {children}
  </div>
);

const DarkCard = ({ children, className = "" }) => (
  <div className={cn("rounded-[1.65rem] border border-white/10 bg-white/[0.04] shadow-2xl shadow-black/20 backdrop-blur", className)}>
    {children}
  </div>
);
...
const LoopStep = ({ n, label, title, body, icon: Icon, active }) => (
  ...
  >
    ...
    {active && <div className="mt-5 inline-flex rounded-full bg-indigo-600 px-3 py-1 text-xs font-semibold text-white">The pivot</div>}
  </div>
);
  ...
  return (
    <SoftCard className="overflow-hidden">
      <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-indigo-600">Pilot evidence</p>
          <h3 className="mt-1 text-xl font-semibold tracking-tight text-slate-950">Live confirmation metrics</h3>
        </div>
        <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">read-only</span>
      </div>
        ...
        <div className="space-y-4">
          <div>
            <div className="mb-1 flex justify-between text-sm">
              <span className="flex items-center gap-2 text-slate-700"><Phone size={14} /> SMS</span>
              <span className="font-semibold text-slate-950">82%</span>
            </div>
            <div className="h-2 rounded-full bg-slate-100"><div className="h-2 w-[82%] rounded-full bg-indigo-600" /></div>
          </div>
          <div>
            <div className="mb-1 flex justify-between text-sm">
              <span className="flex items-center gap-2 text-slate-700"><Mail size={14} /> Email</span>
              <span className="font-semibold text-slate-950">61%</span>
            </div>
            <div className="h-2 rounded-full bg-slate-100"><div className="h-2 w-[61%] rounded-full bg-cyan-400" /></div>
          </div>
        </div>
      </div>
    </SoftCard>
...
const Walkthrough = () => {
  ...
  const content = {
    buyer: {
      ...
      note: "Pilot starts as a manual payment-intent queue. No ERP integration required.",
    },
    vendor: {
      ...
      note: "The vendor is a counterparty signer in v1, not a full CAST customer tenant.",
    },
    proof: {
    ...
    <GradientShell>
      <div className="grid gap-0 lg:grid-cols-[0.78fr_1.22fr]">
        <div className="border-b border-white/10 p-7 lg:border-b-0 lg:border-r lg:p-9">
          ...
          <p className="mt-5 text-base leading-7 text-slate-400">Follow one invoice from payment intent to vendor confirmation to retained proof. This replaces the PO walkthrough without changing the CAST visual language.</p>
            ...
            {tabs.map((t) => (
              <button
                ...
                className={cn(
                  ...
                  tab === t.id ? "bg-white text-slate-950" : "text-slate-400 hover:text-white"
                )}
              >
                {t.label}
              </button>
          ...
          <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.035]">
            {selected.rows.map((row, i) => (
              <div key={i} className="grid grid-cols-4 gap-3 border-b border-white/10 px-4 py-4 text-sm last:border-b-0">
                {row.map((cell, j) => (
                  <div key={j} className={j === 0 ? "font-semibold text-white" : "text-slate-400"}>{cell}</div>
                ))}
              </div>
            ))}
          </div>
    ...
    <div className="min-h-screen overflow-hidden bg-[#f6f9fc] text-slate-700 selection:bg-indigo-600 selection:text-white">
      ...
      <header className="relative z-50 border-b border-white/60 bg-white/70 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
          ...
          <a href="mailto:peter@digitalfinancehq.com?subject=CAST%20Confirm%20%26%20Pay%20pilot" className="group inline-flex items-center gap-2 rounded-full bg-slate-950 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-slate-300/50 transition hover:bg-indigo-600">
      ...
      <main id="top" className="relative z-10">
        <section className="mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-20 md:grid-cols-[1.02fr_0.98fr] md:pb-28 md:pt-28">
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
            ...
            <h1 className="max-w-4xl text-6xl font-semibold tracking-[-0.065em] text-slate-950 md:text-7xl lg:text-8xl">
              Confirm vendor payments before they move.
            </h1>
            <p className="mt-7 max-w-2xl text-xl leading-8 text-slate-600">
              CAST adds a bilateral gate to high-risk AP payments. Before release, the vendor confirms the amount, invoice, payment date, and destination-account last four.
            </p>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <a href="#pilot" className="group inline-flex items-center justify-center gap-2 rounded-full bg-indigo-600 px-6 py-3 text-sm font-semibold text-white shadow-xl shadow-indigo-200 transition hover:bg-slate-950">
                Run a 30-day pilot <ArrowRight size={17} className="transition group-hover:translate-x-0.5" />
              </a>
              <a href="#how" className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-semibold text-slate-950 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50">
        ...
        <section className="relative border-y border-slate-200 bg-white/70">
          <div className="mx-auto grid max-w-7xl gap-8 px-5 py-8 md:grid-cols-4">
            ...
            <Stat value="1k" label="Bilateral events to prove the wedge" />
          </div>
        </section>
        ...
        <section id="how" className="mx-auto max-w-7xl px-5 py-24">
          ...
          <div className="mb-12 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <h2 className="max-w-4xl text-5xl font-semibold tracking-[-0.055em] text-slate-950 md:text-6xl">
              One party proposes. The vendor confirms. Policy governs release.
            </h2>
            <p className="max-w-md text-base leading-7 text-slate-600">This is the entire first product: AP Confirm & Pay. No mirrored seller ledgers. No PO-confirmation network. No full ERP replacement.</p>
          </div>
        ...
        <section id="security" className="mx-auto max-w-7xl px-5 py-24">
          <SectionKicker>Covered AP triggers</SectionKicker>
          <div className="mb-12 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            ...
            <p className="max-w-md text-base leading-7 text-slate-600">The first pilot does not need to cover every invoice. It covers the flows where unilateral instructions create the most damage.</p>
          </div>
        ...
        <section id="pilot" className="relative border-y border-slate-200 bg-white py-24">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 md:grid-cols-[0.86fr_1.14fr] md:items-center">
            <div>
              <SectionKicker>30-day pilot</SectionKicker>
              <h2 className="text-5xl font-semibold tracking-[-0.055em] text-slate-950 md:text-6xl">Prove the wedge with real payment confirmations.</h2>
              <p className="mt-6 text-lg leading-8 text-slate-600">Start with 25–100 high-risk AP payments. Measure whether vendors confirm quickly enough for the control to become operationally real.</p>
              <div className="mt-8 space-y-3 text-sm font-medium text-slate-700">
                {["No ERP integration required", "SMS primary when phone is verified", "Email fallback when SMS fails", "Evidence dashboard for investor and customer demos"].map((item) => (
        ...
        <section className="mx-auto max-w-7xl px-5 py-24">
          <SectionKicker>Build tomorrow</SectionKicker>
          <div className="mb-12 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <h2 className="max-w-4xl text-5xl font-semibold tracking-[-0.055em] text-slate-950 md:text-6xl">The first product is the bilateral event loop.</h2>
            <p className="max-w-md text-base leading-7 text-slate-600">Everything below is intentionally narrower than the full CAST architecture.</p>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            <SoftCard className="p-6">
              <Database className="mb-6 text-indigo-600" size={24} />
              <h3 className="text-xl font-semibold tracking-tight text-slate-950">Data spine</h3>
              <p className="mt-3 text-sm leading-6 text-slate-600">Implement events, vendors, work_orders, and policy_versions. Defer postings and cases.</p>
              <div className="mt-6 space-y-2 rounded-2xl bg-slate-50 p-4 font-mono text-xs text-slate-500"><div>events.append_only</div><div>work_orders.bilateral_state</div><div>vendors.confirmation_channel</div><div>policy_versions.active</div></div>
            </SoftCard>
            <SoftCard className="p-6">
              <GitBranch className="mb-6 text-indigo-600" size={24} />
              <h3 className="text-xl font-semibold tracking-tight text-slate-950">Confirmation flow</h3>
              <p className="mt-3 text-sm leading-6 text-slate-600">Buyer creates payment intent. CAST sends Trade Message. Vendor confirms, requests change, or disputes.</p>
              <div className="mt-6 space-y-2 rounded-2xl bg-slate-50 p-4 font-mono text-xs text-slate-500"><div>trade_message.sent</div><div>trade_message.viewed</div><div>vendor.confirmed</div><div>vendor.disputed</div></div>
            </SoftCard>
            <SoftCard className="p-6">
              <BarChart3 className="mb-6 text-indigo-600" size={24} />
              <h3 className="text-xl font-semibold tracking-tight text-slate-950">Evidence dashboard</h3>
              <p className="mt-3 text-sm leading-6 text-slate-600">Track the metrics that decide whether this deserves the next build phase.</p>
              <div className="mt-6 space-y-2 rounded-2xl bg-slate-50 p-4 font-mono text-xs text-slate-500"><div>confirmation_rate</div><div>median_confirmation_time</div><div>request_change_rate</div><div>non_response_rate</div></div>
            </SoftCard>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-5 pb-28">
          <GradientShell>
            <div className="grid gap-10 p-8 md:grid-cols-[1fr_0.86fr] md:items-center md:p-10">
              <div>
                <SectionKicker dark>Scope boundary</SectionKicker>
                <h2 className="text-4xl font-semibold tracking-[-0.05em] text-white md:text-6xl">What this version does not claim.</h2>
                <p className="mt-6 text-lg leading-8 text-slate-400">Confirm & Pay is not an ERP replacement, not seller-side AR automation, not a PO-confirmation network, and not a guarantee against collusion. It is a narrow pre-payment control for covered AP flows.</p>
              </div>
